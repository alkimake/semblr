...
**Solution:** A persistent repository of turns (each user prompt + model response sequence). Embedded and retrievable by semantic similarity.
...
**Solution:** Dynamic context assembly from the turn repository. Each turn gets a fixed budget. Always the most relevant turns, never lossy summarization.

## Architecture

### Turn Repository
- Each turn is saved as an individual file
- A "turn" = one user message → full model response sequence (thinking, tool calls, tool results, final answer, etc.)
- Files are append-only, never modified after creation

### Embedding Index
- Each turn gets two embeddings: prompt vector + response vector
...
4. Pull in turns until the token budget is reached
5. Construct context: system prompt + retrieved turns + current prompt
6. Send to the model
...
- **Turn repository:** Flat files on disk
- **Embeddings:** Any embedding API (tbd)
- **Vector index:** Flat file + numpy
- **Context assembly:** Extension hooks (`before_agent_start`, `context`, `turn_end`, `agent_end`)

## Design Principles

1. **Semantic over sequential.** Relevance beats recency.
2. **Fixed-size context.** Predictable, cache-friendly windows.
3. **Append-only repository.** Never modify a saved turn. Build better retrieval instead.
...
- [ ] Pi extension that wipes context clean on every turn ("total amnesia")
...
### Phase 2 — Turn Repository
- [ ] Save each completed turn to a file on disk
- [ ] Structure: `turns/<id>.json` with prompt, response sequence, timestamps
- [ ] Embed prompt and response separately
- [ ] Maintain vector index file

### Phase 3 — Retrieval
- [ ] On `before_agent_start`, embed incoming prompt
- [ ] Query index by distance
- [ ] Assemble context from closest turns up to token budget
...
### Phase 5 — Advanced
- [ ] Multiple embedding strategies per turn
- [ ] Hybrid retrieval (semantic + keyword/BM25)
- [ ] User-directed context curation
- [ ] Cross-project turn repository sharing
